...
def miti_estimator(circuit,operator,estimator,shots=10000,level_miti=1):
  exp_vals=[]
  exp_errors=[]
  ...
  for i, pauli_op in enumerate(operators):
    ...
    if level_miti==0:
      exp_val=bootstrap_mitigated_expectation(result_measure,diag_pauli_op,20,shots,complete_meas_fitter=None)
      exp_vals.append(exp_val[0])
      exp_errors.append(exp_val[1])
    if level_miti==1:
      exp_val=bootstrap_mitigated_expectation(result_measure,diag_pauli_op,20,shots,complete_meas_fitter=meas_fitter)
      exp_vals.append(exp_val[0])
      exp_errors.append(exp_val[1])
    if level_miti==2:
      miti_counts=bootstrap_counts(result_measure,20,shots,return_mean=True,complete_meas_fitter=meas_fitter)
      exp_val=bootstrap_mitigated_expectation(miti_counts,diag_pauli_op,20,shots,complete_meas_fitter=meas_fitter_GEM)
      exp_vals.append(exp_val[0])
      exp_errors.append(exp_val[1])
    if level_miti==3:
      exp_val=bootstrap_mitigated_expectation(result_measure,diag_pauli_op,20,shots,complete_meas_fitter=meas_fitter)
    ...
    if level_miti==4:
      ...
      exp_val=bootstrap_mitigated_expectation(c,diag_pauli_op,20,shots,complete_meas_fitter=meas_fitter_ampdep)
      exp_vals.append(exp_val[0])

  if coefficients is None:
    return exp_vals,exp_errors
  else:
    exp_val=0
    exp_err=0
